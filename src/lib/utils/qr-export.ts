...
interface CompactTask {
  ...
  c: string; // category
  n: string; // notes
  a: number | string | null; // assignee_id
  sp: number | string | null; // sprint_id
...
interface CompactData {
  v: 1; // version
  tasks: CompactTask[];
  projects: Array<{ i: number | string; n: string }>;
  assignees: Array<{ i: number | string; n: string; c: string }>;
}
...
export function getRelatedData(
  ...
  const projectNames = new Set(
    selectedTasks.map((t) => t.project).filter(Boolean),
  );
  ...
  const assignees = allAssignees.filter(
    (a) => a.id != null && assigneeIds.has(a.id),
  );
...
export function estimateQRSize(data: string): {
  bytes: number;
  fitsQR: boolean;
  percentage: number;
} {
