...
export type TldrawBridgeSyncStatus = 'offline' | 'connecting' | 'online' | 'error';
...
function OfflineCanvas({ initialSnapshot, onEditorReady, onUserSnapshot }: OfflineCanvasProps) {
...
function SyncCanvas({
	...
	if (store.status !== 'synced-remote') {
		return createElement('div', {
			...
			children: store.status === 'error' ? 'ไม่สามารถเชื่อมต่อ live sync ได้' : 'กำลังเชื่อมต่อ live sync...'
		});
	}
