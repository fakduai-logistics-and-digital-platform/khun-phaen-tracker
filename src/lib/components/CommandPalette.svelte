<script lang="ts">
	...
	import type { Task, Project, Assignee, FilterOptions, CommandPaletteItem } from '$lib/types';
	import { getFuzzyScore, normalizeForCommandSearch } from '$lib/utils/command-palette';
	...
	function getCommandCategoryLabel(category: CommandPaletteItem['category']): string {
		switch (category) {
			case 'command': return t('commandPalette__cat_commands');
			case 'search': return t('commandPalette__cat_search');
			case 'task': return t('commandPalette__cat_tasks');
			case 'project': return t('commandPalette__cat_projects');
			case 'assignee': return t('commandPalette__cat_assignees');
			case 'sprint': return t('commandPalette__cat_sprints');
			default: return '';
		}
	}
	...
	$: dynamicCommandPaletteItems = (() => {
		...
		for (const task of tasks) {
			...
			const sprintName = task.sprint_id ? sprintNameById.get(String(task.sprint_id)) || '' : '';
			...
			taskItems.push({
				...
				item: {
					id: `task-${task.id}`,
					label: t('commandPalette__open_task_label', { values: { title: task.title } }),
		...
		for (const project of projects) {
			...
			projectItems.push({
				...
				item: {
					...
					label: t('commandPalette__filter_project_label', { values: { name: project.name } }),
		...
		for (const assignee of assignees) {
			...
			assigneeItems.push({
				...
				item: {
					...
					label: t('commandPalette__filter_assignee_label', { values: { name: assignee.name } }),
		...
		for (const sprint of sprints) {
			...
			sprintItems.push({
				...
				item: {
					...
					label: t('commandPalette__go_to_sprint_label', { values: { name: sprint.name } }),
					description: t('commandPalette__go_to_sprint_desc', { values: { name: sprint.name } }),
		...
		const quickSearchItem: CommandPaletteItem = {
			...
			label: t('commandPalette__search_tasks_label', { values: { query: commandQuery.trim() } }),
	...
	function getCommandStatusInfo(status: string) {
		switch (status) {
			case 'done':
				return { label: t('page__filter_status_done'), class: 'bg-green-50 text-green-600 border-green-100 dark:bg-green-500/10 dark:text-green-400 dark:border-green-500/20' };
			case 'in-progress':
				return { label: t('page__filter_status_in_progress'), class: 'bg-blue-50 text-blue-600 border-blue-100 dark:bg-blue-500/10 dark:text-blue-400 dark:border-blue-500/20' };
			case 'in-test':
				return { label: t('page__filter_status_in_test'), class: 'bg-purple-50 text-purple-600 border-purple-100 dark:bg-purple-500/10 dark:text-purple-400 dark:border-purple-500/20' };
			default:
				return { label: t('page__filter_status_todo'), class: 'bg-amber-50 text-amber-600 border-amber-100 dark:bg-amber-500/10 dark:text-amber-400 dark:border-amber-500/20' };
		}
	}
</script>
	...
	>
		<div class="w-full max-w-2xl rounded-2xl border border-gray-200 dark:border-gray-700 bg-white/95 dark:bg-gray-900/95 shadow-2xl overflow-hidden">
			<div class="px-4 py-3 border-b border-gray-100 dark:border-gray-800 flex items-center justify-between">
				<p class="text-sm font-semibold text-gray-900 dark:text-gray-100">⌨️ {t('commandPalette__title')}</p>
