...
* All read and write operations from the UI must flow through the Svelte stores or the database client helper [db.ts](file:///Users/watchakorn.b/FILE_CODE/OTHER/task-tracker-offline/src/lib/db.ts).
...
* Ensure all IndexedDB changes are covered by unit tests similar to [db.unit.test.ts](file:///Users/watchakorn.b/FILE_CODE/OTHER/task-tracker-offline/src/lib/db.unit.test.ts).
